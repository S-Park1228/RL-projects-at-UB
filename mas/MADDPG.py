...
#             self.buffers[agent_id] = ReplayBuffer(capacity, obs_dim, act_dim, 'cpu')
...
#             d = done[agent_id]
#             self.buffers[agent_id].push(o, a, r, next_o, d)
...
class MADDPG:
    ...
    Unlike single-agent cases, split the RL agent into two.
    First, MADDPG manages the interaction between the environment and agents.
    Secondly, each "Agent" instance manages its own networks and target networks.
    """

    def __init__(self, dim_info, capacity, batch_size, actor_lr, critic_lr, result_dir):
        # sum all the dims of each agent to get input dim for critic
        global_obs_act_dim = sum(sum(val) for val in dim_info.values())
        self.agents = {}
        self.buffers = {}
        ...
        self.result_dir = result_dir  # directory to save the training results
        self.logger = setup_logger(os.path.join(result_dir, 'maddpg.log'))
    ...
    def push(self, obs, actions, rewards, next_obs, dones):
        ...
        for agent_id in obs.keys():
            ...
            a = actions[agent_id]
            if isinstance(a, int):
                # If the action from env.action_space(agent_id).sample() has int datatype,
                # it needs to be converted into one-hot.
                a = np.eye(self.dim_info[agent_id][1])[a] # self.dim_info[agent_id][1]: the action dimension of the corresponding agent
    ...
    def get_actions(self, obs):
        ...
        for agent, o in obs.items():
            o = torch.from_numpy(o).unsqueeze(0).float()
            a = self.agents[agent].action(o)  # torch.Size([1, action_size])
            # NOTE that 'a' is a tensor. Thus, convert it into int before insert it into the environment.
            actions[agent] = a.squeeze(0).argmax().item()
            self.logger.info(f'{agent} action: {actions[agent]}')
    ...
    def sample(self, batch_size):
        ...
        """
        Sample experience from all the agents' buffers.
        """
        
        # get the total num of transitions, these buffers should have same number of transitions
        buffer_size = len(self.buffers['agent_0'])
        ...
        for agent_id, buffer in self.buffers.items():
            ...
            # Additionally, the next action is necessary for update.
            next_act[agent_id] = self.agents[agent_id].target_action(n_o)
    ...
    def update(self, batch_size, gamma):
        for agent_id, agent in self.agents.items():
            ...
            # Note that the actor-network in this case follows an on-policy.
            # Thus, the actions from the replay buffer must not be inserted.
            action, logits = agent.action(obs[agent_id], model_out = True)
            act[agent_id] = action
            actor_loss = -agent.critic_value(list(obs.values()), list(act.values())).mean()
            actor_loss_pse = torch.pow(logits, 2).mean()
    ...
    def save(self, rewards):
        ...
        """
        Save the model (actor-network parameters of all agents ) and its training performance in `result_dir`.
        """
        
        torch.save(
            {name: agent.actor.state_dict() for name, agent in self.agents.items()},  # actor-network parameters
            os.path.join(self.result_dir, 'maddpg_actor_for_mpe.pt')
        )
        with open(os.path.join(self.result_dir, 'maddpg_rewards_for_mpe.pkl'), 'wb') as file:  # Save the model's training performance.
            pickle.dump({'rewards': rewards}, file)
